// ─────────────────────────────────────────────────────────────────────────────
// lib/partner-calls-handlers.ts
// app/api/partner/calls/route.ts 에서 추출한 핸들러 로직
// ─────────────────────────────────────────────────────────────────────────────
...
export function isMissingColumnError(
  ...
  return (
    error?.code === "42703" ||
    /does not exist|column .* does not exist/i.test(error?.message ?? "")
  );
}

export function hyphenKoreanMobile(digits: string): string {
...
export async function resolveApprovedDriver(): Promise<DriverContext> {
  ...
  const p = profile as { role?: unknown; partner_driver_id?: unknown } | null | undefined;
  ...
  if (
    !driver ||
    safeText((driver as { status?: unknown }).status, "").toLowerCase() !== "approved"
  ) {
...
export async function handlePartnerCallsGet(
  driver: Extract<DriverContext, { ok: true }>,
): Promise<
  | {
      ok: true;
      calls: unknown[];
      service_regions: string[];
      service_regions_required: boolean;
      driver: { company_name: string; manager_name: string; phone: string };
    }
  | { error: string; status: number }
> {
  const admin = createServiceRoleSupabase();
  if (!admin) {
    return { error: "SUPABASE_SERVICE_ROLE_KEY 가 설정되지 않았습니다.", status: 503 };
  }
  ...
  if (applicationsError) {
    return { error: applicationsError.message, status: 502 };
  }
  ...
  const rows =
    driver.serviceRegions.length === 0
      ...
      : rawRows.filter((raw) => {
          ...
          return departureRegion !== "" && driver.serviceRegions.includes(departureRegion);
        });
  const ids = rows
  ...
  if (ids.length > 0) {
    const [{ data: memberCountRows }, { data: guestCountRows }, { data: sponsorRows }] =
      await Promise.all([
    ...
    for (const raw of Array.isArray(sponsorRows) ? sponsorRows : []) {
      const companyId = safeText(
        (raw as { sponsor_company_id?: unknown }).sponsor_company_id,
        "",
      );
    ...
    for (const raw of Array.isArray(memberCountRows) ? memberCountRows : []) {
      const applicationId = safeText(
        (raw as { application_id?: unknown }).application_id,
        "",
      );
    ...
    if (
      isMissingColumnError(memberQuotesResult.error) &&
      /support_breakdown/i.test(memberQuotesErrMsg)
    ) {
    ...
    if (memberQuotesError) {
      return { error: memberQuotesError.message, status: 502 };
    }
    ...
    for (const q of Array.isArray(memberQuotes) ? memberQuotes : []) {
      ...
      quotedByApplication.set(applicationId, {
        ...
        sponsor_approved_support_amount:
          supportFields.total_confirmed_support ?? sponsorApproved,
    ...
    if (driver.phoneDigits !== "") {
      const guestPhones = [driver.phoneDigits, hyphenKoreanMobile(driver.phoneDigits)];
      ...
      if (guestQuotesError) {
        return { error: guestQuotesError.message, status: 502 };
      }
  ...
  const calls = await Promise.all(
    rows.map(async (raw) => {
      ...
      const supportEstimate = estimateSponsorSupport({ passengerCount, price: 0 });
