/**
 * partner/quotes API 핸들러 함수 모음
 * route.ts에서 분리된 POST/PATCH 액션별 로직
 */
...
export async function resolveApprovedDriver(): Promise<
...
export async function handlePartnerQuotePost(
  admin: SupabaseClient,
  driver: { userId: string; partnerDriverId: string; phoneDigits: string },
  body: PartnerQuoteBody,
  ...
  if (vehicleType === "") {
    return NextResponse.json({ error: "차량유형을 입력해 주세요." }, { status: 400 });
  }
  if (availableTime === "") {
    return NextResponse.json({ error: "가능 출발시간을 입력해 주세요." }, { status: 400 });
  }
  ...
  const customerPlannedSupport = requestedSupportDiscountAmount ?? supportInputLimit;
  ...
  if (existingMemberError) {
    return NextResponse.json({ error: existingMemberError.message }, { status: 502 });
  }
  if (existingMember) {
  ...
  if (driver.phoneDigits !== "") {
    ...
    if (existingGuestError) {
      return NextResponse.json({ error: existingGuestError.message }, { status: 502 });
    }
    if (existingGuest) {
  ...
  if (
    ...
    const minimalSupport = await admin
      ...
      .select(
        "id, price, support_discount_amount, customer_support_amount, member_price, sponsor_discounted_price",
      )
      .single();
  ...
  if (insertError && /member_price|does not exist|42703/i.test(insertError.message)) {
...
export async function handlePartnerQuotePatch(
  admin: SupabaseClient,
  driver: { userId: string; partnerDriverId: string; phoneDigits: string },
  body: PartnerQuoteBody,
): Promise<NextResponse> {
  ...
  if (!isApplicationQuoteAccepting(application as unknown as Record<string, unknown>)) {
    return NextResponse.json(
  ...
  if (
    ...
    const colMatch = /column ['"]?(\w+)['"]? (does not exist|of ')/i.exec(
      updateError.message,
    );
    if (colMatch?.[1]) {
