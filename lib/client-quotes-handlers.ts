// ─────────────────────────────────────────────────────────────────────────────
// lib/client-quotes-handlers.ts
// app/api/client/quotes/route.ts 에서 추출한 핸들러 로직
// ─────────────────────────────────────────────────────────────────────────────
...
const APPLICATION_SPONSOR_AND_SELECTED_PRICE_COLUMNS =
  "sponsor_support_status, sponsor_approved_support_amount, sponsor_preapproved_count, sponsor_approved_count, sponsor_rejected_count, client_price_selection_kind, selected_price_type, selected_price_label, selected_price, support_breakdown_snapshot";
...
export async function resolveApplication(
  ...
  if (!admin)
    return { error: "SUPABASE_SERVICE_ROLE_KEY 가 설정되지 않았습니다.", status: 503 } as const;
...
export async function resolveApplicationsByLookupPassword(
  admin: Admin,
  request: Request,
) {
  const { searchParams } = new URL(request.url);
...
export async function loadPayload(admin: Admin, app: Record<string, unknown>) {
  ...
  let memberResult: {
    ...
    .from("driver_quotes")
    .select(
      "id, created_at, application_id, partner_driver_id, auth_user_id, price, vehicle_type, available_time, message, status, support_breakdown",
    )
  ...
  const guestResult: {
    ...
    .select(
      "id, created_at, application_id, price, vehicle_type, available_time, message, status, match_result, guest_company_name, guest_driver_name, guest_phone",
    )
    .eq("application_id", applicationId)
    .order("created_at", { ascending: false });
  ...
  const hasRejectedSponsor =
    ...
    sponsorRows.every((row) =>
      ["rejected", "cancelled", "expired"].includes(safeText(row.status)),
    );
  ...
  const quotes = [
    ...(Array.isArray(memberRows) ? memberRows : []).map((raw) => {
      ...
      const quoteSponsorStatus =
        safeText(row.sponsor_support_status) === "approved" ||
  ...
  return {
    application: {
      ...
      final_price_selection_kind:
        ...
        (safeText(current.selected_price_type)
          ? selectedPriceTypeToLegacyKind(
              safeText(current.selected_price_type) as
                | "normal"
                | "support_planned"
                | "support_confirmed",
            )
      ...
      support_breakdown_snapshot:
        (current as Record<string, unknown>).support_breakdown_snapshot ?? null,
    },
    quotes,
  };
}

export async function handleClientQuotesPost(
  admin: Admin,
  ...
  | { ok: true; payload: Awaited<ReturnType<typeof loadPayload>> }
...
> {
  ...
  if (action === "reopen") {
    await admin
      ...
      .eq("id", applicationId);
    return { ok: true, payload: await loadPayload(admin, app) };
  }
  ...
  if (action === "final_confirm") {
    ...
    if (bodyApplicationId !== "" && bodyApplicationId !== applicationId) {
      return { error: "견적요청 정보가 일치하지 않습니다.", status: 400 };
    }
    const selectedId = quoteId || safeText(app.auto_selected_quote_id);
    const selectedSource = quoteId
      ? quoteSource
      : safeText(app.auto_selected_quote_source) === "guest"
        ? "guest"
        : "member";
    if (selectedId === "") {
      return { error: "확정할 견적이 없습니다.", status: 400 };
    }
    ...
    if (coreUpdate.error) {
      if (isMissingColumnError(coreUpdate.error)) {
        const fallbackCore = await admin
          .from("applications")
          .update(corePatch)
          .eq("id", applicationId);
        if (fallbackCore.error) {
          return { error: fallbackCore.error.message, status: 502 };
        }
        ...
        if (kindUpdate.error && !isMissingColumnError(kindUpdate.error)) {
          return { error: kindUpdate.error.message, status: 502 };
        }
      } else {
        return { error: coreUpdate.error.message, status: 502 };
      }
    }
    ...
    if (selectedSource === "guest") {
      ...
      driverName = safeText(
        guest.guest_driver_name,
        safeText(guest.guest_company_name, "기사님"),
      );
    } else {
      ...
      driverName = safeText(
        partnerRow.manager_name,
        safeText(partnerRow.company_name, "기사님"),
      );
    }
    if (driverPhone) {
  ...
  return { error: "지원하지 않는 동작입니다.", status: 400 };
}
