/**
 * admin/driver-quotes API 핸들러 함수 모음
 * route.ts에서 분리된 GET/PATCH 액션별 로직
 */
...
export async function handleAdminDriverQuotesGet(
  ...
  if (partnerIds.length > 0) {
    const { data: partnersRaw, error: partnersError } = await admin
      ...
      .in("id", partnerIds);
    if (partnersError) {
      return NextResponse.json({ error: partnersError.message }, { status: 502 });
    }
  ...
  let normalized = quotes.map((raw) => {
    ...
    return {
      ...
      sponsor_approved_support_amount:
        supportFields.total_confirmed_support ?? appApprovedTotal,
    ...
    ? Array.from(
        new Set(
          guestRows
            .map((raw) =>
              safeText(
                (raw as { linked_partner_driver_id?: unknown }).linked_partner_driver_id,
                "",
              ),
            )
            .filter(Boolean),
  ...
  const guestLinkPartnerById = new Map<string, { company_name: string; phone: string }>();
...
export async function handleAdminQuoteEdit(
  admin: SupabaseClient,
  ...
  const { data: beforeRow } = await admin
    ...
    .select(
      isMember
        ? "price, support_settlement_type, message, status, application_id"
        : "price, message, status, application_id",
    )
    .eq("id", quoteId)
    .maybeSingle();
  ...
  if (quotePatch.vehicle_type !== undefined)
    patch.vehicle_type = safeText(quotePatch.vehicle_type);
  if (quotePatch.available_time !== undefined)
    patch.available_time = safeText(quotePatch.available_time);
...
export async function handleAdminHideApplication(
  admin: SupabaseClient,
  adminEmail: string | null | undefined,
  applicationId: string,
): Promise<NextResponse> {
...
// ---------------------------------------------------------------------------
// PATCH 핸들러 — 관리자 액션 (reopen / manual_close / final_confirm)
// ---------------------------------------------------------------------------

export async function handleAdminApplicationAction(
  admin: SupabaseClient,
  action: string,
  applicationId: string,
): Promise<NextResponse> {
  ...
  const selectedSource =
    safeText(row?.auto_selected_quote_source) === "guest" ? "guest" : "member";
  if (selectedId === "") {
    return NextResponse.json(
...
// ---------------------------------------------------------------------------
// PATCH 핸들러 — 비회원 견적 match_result 변경
// ---------------------------------------------------------------------------

export async function handleAdminGuestQuoteMatchResult(
  admin: SupabaseClient,
  guestQuoteId: string,
  matchResult: string,
): Promise<NextResponse> {
